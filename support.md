---
layout: page
title: Support
---

Aftervoice is made by one person. Questions, faults and suggestions all go to the same place:

[support@altrovelabs.net](mailto:support@altrovelabs.net)

Write in Italian or English; either is fine.

## Reporting generated audio

Aftervoice makes syllables, not messages. If a combination sounds offensive, discriminatory or
otherwise inappropriate, write to the support address above. The report is about the app's
output; it is not presented as a message from a person or a presence.

You do not need to send a recording. If you choose to attach one, the archive's Share action
exports the app-generated track and does not include the separate room microphone track.

## Common questions

**The app asks to download a voice bank before I can start.** Each language's bank is a model
of about 120 to 140 MB, and it is not inside the app because most people want one language, not
six. It downloads once, over Google Play, and after that the app works with no network at all.

**Can I use it without a network?** Yes, after that first download. There is no other moment
when the app wants one, and it has no permission to use one.

**Why does it ask for the microphone?** To record the room beside the app's own sound when you
press Registra, and for the sonar screen. Refuse it and the app still works; it records only
its own track and says so.

**Where did my diary go after I changed phone?** It stayed on the old one. The app is excluded
from cloud backup and from device-to-device transfer, deliberately: recordings of a room with
people in it are not something to copy around by default.

**The voices sound like nonsense.** They are meant to. Aftervoice makes syllables, not words:
the meaning is what you hear in them, which is the whole tradition it comes from.

## Privacy

[Italiano](privacy/it.html) · [English](privacy/en.html) · [Español](privacy/es.html) ·
[Français](privacy/fr.html) · [Deutsch](privacy/de.html) · [Português (Brasil)](privacy/pt-BR.html)
